...
public class BankAccounts {
    ...
    public static void transferMoney(Account fromAccount,
        ...
        int fromNo = fromAccount.getAcctNo(),
                toNo = toAccount.getAcctNo();
        if (fromNo < toNo) {
            synchronized (fromAccount) {
                synchronized (toAccount) {
                    transferMoneyUtil(fromAccount, toAccount, amount);
                }
            }
        } else {
            synchronized (toAccount) {
    ...
    private static void transferMoneyUtil(Account fromAccount,
                                          Account toAccount,
                                          DollarAmount amount)
            throws InsufficientFundsException {
